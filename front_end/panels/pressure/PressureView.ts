...
export class PressureView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    // debug
    const groupTitle = UI.UIUtils.createLabel(this.#pressureState, 'pressure-group-title');
  ...
  private createCPUSection(): void {
    ...
    // Fair
    this.pressureSelectElement.appendChild(
      UI.UIUtils.createOption(fairOption.title, fairOption.state, 'fair'));
  ...
  private async pressureSelectChanged(): Promise<void> {
    ...
    const prevPressureOverrideEnabled = this.#pressureOverrideEnabled;
    this.#pressureOverrideEnabled = this.#pressureState != 'no-override';

    this.#pressureSetting.set(this.#pressureState);

    if (prevPressureOverrideEnabled != this.#pressureOverrideEnabled) {
